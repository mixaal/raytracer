/*
	...
	- changes 02/01/15: multicore support using openmp library; Makefile added
...
#ifndef _SCREEN_WIDTH
#	define _SCREEN_WIDTH 1920
#endif

#ifndef _SCREEN_HEIGHT
...
static u_int32_t _screen[_SCREEN_HEIGHT * _SCREEN_WIDTH];
...
{
	unsigned width = _SCREEN_WIDTH, height = _SCREEN_HEIGHT;
	//Vec3<T> *image = new Vec3<T>[width * height];
...
#pragma omp parallel for
	...
	for (unsigned y = scanline; y < height; y+=16) {
		for (unsigned x = 0; x < width; ++x) {
			...
			raydir.normalize();
			Vec3<T> pixel = trace(Vec3<T>(0), raydir, spheres, 0);
			//image[width*y + x] = pixel;
			unsigned char r = (unsigned char)(std::min(T(1), pixel.x) * 255);
			unsigned char g = (unsigned char)(std::min(T(1), pixel.y) * 255);
			unsigned char b = (unsigned char)(std::min(T(1), pixel.z) * 255);
			unsigned char a = 0xff;
			_screen[width * y + x] = (a<<24) | (r<<16) | (g<<8) | b;
		}
	}
